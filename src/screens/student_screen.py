...
def student_screen():
    ...
    with c2:
        if st.button("Go back to Home", type='secondary', key='loginbackbtn',shortcut="control+backspace"):
            st.session_state['login_type']=None
            st.rerun()
    st.header("Login Using FaceID", text_alignment='center', anchor="register-teacher")
    st.space()
    st.space()
    ...
    if show_registration:
        with st.container(border=True):
            ...
            new_name = st.text_input("Enter your name", placeholder='E.g. Ayush Kumar')
            ...
            try:
                audio_data = st.audio_input('Record a short phrase like I am present, My name is Ayush.')
            except Exception:
