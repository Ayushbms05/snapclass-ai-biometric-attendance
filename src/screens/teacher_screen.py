...
def teacher_dashboard():
    ...
    with c2:
        ...
        if st.button("Logout", type='secondary', key='registerbackbtn',shortcut="control+backspace"):
    ...
    if st.session_state.current_teacher_tab == "take_attendance":
        teacher_tap_take_attendance()
    if st.session_state.current_teacher_tab == "manage_subjects":
        teacher_tap_manage_subjects()
    if st.session_state.current_teacher_tab == "attendance_record":
        teacher_tap_attendance_record()
    

    footer_dashboard()

def teacher_tap_take_attendance():
    st.header("Take AI Attendance")


def teacher_tap_manage_subjects():
    ...
    if subjects:
        ...
        def share_btn():
            if st.button(f"Share Code: {sub['name']}", key=f"share_{'subject_code'}", icon=":material/share:"):
...
def teacher_tap_attendance_record():
...
def teacher_screen_login():
    ...
    with c2:
        if st.button("Go back to Home", type='secondary', key='loginbackbtn',shortcut="control+backspace"):
            st.session_state['login_type']=None
            st.rerun()
    st.header("Login Using Password", text_alignment='center', anchor="register-teacher")
    st.space()
    st.space()
...
def teacher_screen_register():
    ...
    st.header("Register your teacher profile", anchor="register-teacher")

    st.space()
    st.space()
    teacher_username = st.text_input("Enter Username", placeholder="@username")
    teacher_name = st.text_input("Enter name", placeholder="Ayush Kumar")
